...
Built-in protection excludes Git metadata, environment files other than `.env.example`, credential files and directories, key files, dependencies, caches, build output, local databases, and local coding-agent configuration. See `snapshot.py` for the complete list. These exclusions apply even to tracked files and cannot be disabled by repository settings. They are path rules, not a secret scanner: do not commit secrets inside ordinary source files.
